...
void metrics_stdout::set_ue_handle(ue_metrics_interface* ue_)
{
  ...
  ue = ue_;
  //outfileue.open("ue_tpt.txt");
  //publisher.bind("ipc:///tmp/socket_ue_tpt"); 
}
                                        ...
                                        bool                 print_carrier_num)
{
  ...
  if(log_flag)
  {
    ...
    if(cqi_change_flag)
      sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/tpt_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
    else
      sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/tpt_ue1_%s.txt",num_ues,algo,load,scenario);
  ...
  //std::string send_message_client("");

  fmt::print(" {:>6.6}", float_to_eng_string((float)mac[r].rx_brate / (mac[r].nof_tti * 1e-3), 2));
  
  //send_message_client += float_to_eng_string((float)mac[r].rx_brate / (mac[r].nof_tti * 1e-3), 2);
  //outfileue << send_message_client << "\n";

  //int m_size = send_message_client.size(); 
  //zmq::message_t message(m_size); 
  //snprintf((char *)message.data(), m_size, send_message_client.c_str(), NULL); 
