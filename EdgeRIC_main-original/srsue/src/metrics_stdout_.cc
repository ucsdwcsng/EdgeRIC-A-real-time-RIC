                                        ...
                                        bool                 print_carrier_num)
{
  ...
  {
    char filename[100];
    sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/SL_%0.1fM_CQI_CHANGE_1IN%d_STEP%d/tpt_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_step_size,scenario);
    if(firstwrite)
    {
      fp = fopen(filename,"w+");
