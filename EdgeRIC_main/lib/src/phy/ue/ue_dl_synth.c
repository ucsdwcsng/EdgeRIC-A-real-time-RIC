 ...
 * and at https://urldefense.com/v3/__http://www.gnu.org/licenses/__;!!Mih3wA!DOe_OPOOfkmga0lyXcHjheSdE6SHZAJAdhKJMHWcDq1ROBGRbbGsCfGkCFaDq7IcTPKe7DBf8eFn$  .
 *
 */
...
//int cqi_range1[2] = {6, 15};  // if cqi_flag == 1  // use this for ue1
int cqi_range1[2] = {1, 15};  // if cqi_flag == 1  // use this for ue1
//int cqi_range2[2] = {1, 10};  // if cqi_flag != 1  // use this for ue2
int cqi_range2[2] = {5, 15};  // if cqi_flag != 1  // use this for ue2
int8_t cqi_flag = 1;
//int cqi_value = 6;   // use this
int cqi_value = 15;   //or 15 use this
// int cqi_offset_val = 0;
// int cqi_offset[2] = {-2,2};
bool cqi_evol_flag = 1;
int period_to_alter_range = 1000; // [msec]  // use this
...
int srsran_ue_dl_init(srsran_ue_dl_t* q, cf_t* in_buffer[SRSRAN_MAX_PORTS], uint32_t max_prb, uint32_t nof_rx_antennas)
{
  ...
  if(cqi_change_flag)
    sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/cqi_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
  else
    sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/cqi_ue1_%s.txt",num_ues,algo,load,scenario);
                                   ...
                                   srsran_uci_data_t*  uci_data)
{
  ...
  if(cqi_evol_flag)
  {
    ...
    if (cqi_value <= cqi_range1[0]) cqi_range_step = -1 ; 
    if (cqi_value >= cqi_range1[1]) cqi_range_step = 1 ;
