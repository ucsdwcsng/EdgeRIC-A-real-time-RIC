...
std::ofstream outfilenewall ;
std::ofstream outfile_seq ;
std::ofstream outfile_seq_5 ;
std::ofstream logacks;

bool __outfile_flag = false ; 
bool __outfile_seq_flag = false ; 
bool __outfile_seq_flag_5 = false ;
               ...
               rrc_interface_mac*       rrc)
{
  ...
  publisher2.bind("ipc:///tmp/socket_rtt_time"); 
  ...
  outfile.open("out_mac.txt"); 
  outfile_seq.open("out_mac_seq_1new.txt"); 
  outfile_seq_5.open("out_mac_seq_5.txt"); 
  outfilenewall.open("out_mac_all.txt");
  logacks.open("acks.txt");
...
void mac::ric_comm(std::map<uint16_t, float>& weights)
{
  ...
  */
  ...
  std::string send_message_to_logger("");
...
void mac::ric_comm(std::map<uint16_t, float>& weights, uint8_t& a, uint8_t& b, std::map<uint16_t, uint32_t>& pending_data_ul)
{
  ...
  */
  ...
  if (size) {
    ...
    //__outfile_flag = true;
    __outfile_flag = true; 
    __outfile_seq_flag = true;
    __outfile_seq_flag_5 = true; 
    struct timeval ctime ; 
    gettimeofday(&ctime, NULL); 
    ...
    //if (__outfile_flag) outfile << std::to_string(utime_cur) + ", "; 
    ///

     // 5. RAN received weight from RIC
    struct timespec ctime_5 ; 
    int ret_1 =clock_gettime(CLOCK_REALTIME, &ctime_5); 
    long utime_cur_system_5 = ctime_5.tv_sec*1000000000 + ctime_5.tv_nsec ; 
    send_message_to_logger += std::to_string(utime_cur_system_5) + " ";
    if (__outfile_seq_flag_5) outfile_seq_5 << std::to_string(utime_cur_system_5) + ",\t" + std::to_string(tti_ran_index) + ",\t" + text + "\n";    


    // 5. RAN receives weight from RIC
    if(text.size() >ue_db.size()*3 ){
      ...
      ric_index = ric_ind;
      text.erase(0, pos_idx + len_delimiter);
      pos_preranind1 = text.find(space_delimiter);
      //pos_preranind2 = text.find("x");

      int preranind = std::stoi(text.substr(0,pos_preranind1));
      preran_index = preranind;
      ...
      //if (__outfile_flag) outfile <<  std::to_string(ric_index) + ", ";
      //printf("\n"); 
      //std::map<uint16_t, float> temp = std::stof(std::to_string(*static_cast<char*>(recv_message.data())));
      //printf("mac: %i,  %s\n",size, (const char*)recv_message.data()); 
    } 
  ...
  // Logging every TTI: 
  struct timespec ctime_5tti ; 
  int ret_2 =clock_gettime(CLOCK_REALTIME, &ctime_5tti); 
  long utime_cur_system_5tti = ctime_5tti.tv_sec*1000000000 + ctime_5tti.tv_nsec ; 
  if (__outfilenewall_flag) outfilenewall << std::to_string(r)+ ", " +std::to_string(utime_cur_system_5tti) + ", 5, \t" + std::to_string(tti_ran_index) + ",\t" ;    
  ...
  for (auto& ue_pair : ue_db) {
    ...
    cqi = __cqis[rnti]; 
    if (__outfilenewall_flag) outfilenewall <<  std::to_string(rnti) + ", " + std::to_string(weights[rnti]) + ", " ;
  ...
  send_message_to_rl += std::to_string(ric_index) + " " + std::to_string(txbytes) + " " + std::to_string(tti_ran_index) + " "; 

  //send_message_to_rl += std::to_string(tti_ran_index) + " "; 

  int m_size = send_message_to_rl.size(); 
  zmq::message_t message(m_size); 
  snprintf((char *)message.data(), m_size, send_message_to_rl.c_str(), NULL ); 
  publisher.send(message, zmq::send_flags::none ); 
  // 1. RAN transmits state to RIC
  //publisher.send(send_message_to_rl.c_str(), send_message_to_rl.size());
  ...
  send_message_to_logger += std::to_string(utime_cur_system_1) + "\n";
  int m_size1 = send_message_to_logger.size(); 
  zmq::message_t message1(m_size1); 
  memcpy(message1.data(), send_message_to_logger.c_str(), m_size1); // copy message to zmq message data
  publisher2.send(message1, zmq::send_flags::none); // send message via publisher2
  //if (__outfile_seq_flag) outfile_seq << std::to_string(utime_cur_system_1) + ",\t" + send_message_to_rl + "\n"; 
  if (__outfilenewall_flag) outfilenewall << std::to_string(utime_cur_system_1) + ", 1, \t" + send_message_to_rl + ",\t";
}
#endif
...
void mac::calFairness(sched_interface::dl_sched_res_t sched_result){
  ...
  if(__outfilenewall_flag) outfilenewall << std::to_string(sum_backlog) + ", " + std::to_string(backlogFairness) + ", " ;
  ...
  for (auto& u : ue_db)
  {
    ...
    if (__outfilenewall_flag) outfilenewall << std::to_string(rnti) + ", " + std::to_string(tx_bytes_ues[rnti]*8.0/1000.0) + ", ";
    str = str + std::to_string(rnti) + " " + std::to_string(tx_bytes_ues[rnti]*8.0/1000.0) + " ";
  ...
  if (__outfilenewall_flag) outfilenewall << std::to_string(tx_bytes*8.0/1000.0) + ", "; 
  txb = tx_bytes*8.0/1000.0;
  //if (__outfilenewall_flag) outfilenewall << std::to_string(txb) + ", ";
  ...
  //txb = _sumthroughput;
  //if (__outfile_flag) outfile << std::to_string(_sumthroughput) + ", " + std::to_string(_thrptFairness) + ", " + std::to_string((int)round(avg_tx_bytes)) + "\r"; 
  if (__outfilenewall_flag) outfilenewall << std::to_string(_sumthroughput) + ", " + std::to_string(_thrptFairness) + ", " + std::to_string((int)round(avg_tx_bytes)) + "\n"; 
  ///
  //txb = _sumthroughput;
