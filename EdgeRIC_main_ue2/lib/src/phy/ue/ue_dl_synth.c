...
//int cqi_range2[2] = {1, 10}; // if cqi_flag != 1  // use this for ue 2
int cqi_range2[2] = {1, 15}; // if cqi_flag != 1  // use this for ue 2
int8_t cqi_flag = 2; 
int cqi_value = 1; // use this
...
int srsran_ue_dl_init(srsran_ue_dl_t* q, cf_t* in_buffer[SRSRAN_MAX_PORTS], uint32_t max_prb, uint32_t nof_rx_antennas)
{
  ...
  char filename[440];
  // sprintf(filename,"/home/afc/git/5G_EdgeRIC_Updated/MobiCom_Results/%dUEs/%s/SL_%0.1fM_CQI_CHANGE_1IN%d_STEP%d/tpt_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_step_size,scenario);
  if(cqi_change_flag)
    sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/cqi_ue2_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
  else
    sprintf(filename,"/home/wcsng-24/Ushasi/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/cqi_ue2_%s.txt",num_ues,algo,load,scenario);
                                   ...
                                   srsran_uci_data_t*  uci_data)
{
  ...
  if(cqi_evol_flag)
  {
    ...
    if (cqi_value <= cqi_range2[0]) cqi_range_step = -1 ; 
    if (cqi_value >= cqi_range2[1]) cqi_range_step = 1 ;
