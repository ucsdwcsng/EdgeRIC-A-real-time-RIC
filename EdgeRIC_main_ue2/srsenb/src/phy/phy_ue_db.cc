...
/// Harish code modifications: Initializing min and max cqi values to configure
// Start
// uint32_t cqi_min_max_vals[4] = {7,15,2,9};
uint32_t cqi_min_max_vals[2] = {14,15};
// uint32_t cqi_min_max_vals[2] = {2,9};
// uint32_t cqi_min_max_vals[2] = {4,12};
// uint32_t cqi_min_max_vals[2] = {6,13};
// uint32_t cqi_min_max_vals[2] = {1,5};
// uint32_t cqi_min_max_vals[2] = {3,11};
// uint32_t cqi_min_max_vals[2] = {5,10};
// uint32_t cqi_min_max_vals[2] = {4,8};
// uint32_t cqi_min_max_vals[2] = {10,15};
// uint32_t cqi_min_max_vals[2] = {2,6};

uint32_t ue_cnt = 0;
uint16_t rnti_vals [10] = {0};
uint16_t cqi_disp_cnt = 0;
// End: Other modifications are from line no:624 in phy_ue_db::send_uci_data function
...
// int cnt = 0; 
// std::map<uint16_t, uint8_t> prev_stack_value  ;
// uint8_t high_stack_value = 6 ; 
// uint8_t low_stack_value = 3 ; 
                             ...
                             stack_interface_phy_lte*       stack)
{
  ...
  // uint8_t  modified_stack_value  = prev_stack_value[rnti]>0 ? prev_stack_value[rnti] : high_stack_value ;

  // cnt++ ; 
  // if (cnt > 2){
  //   modified_stack_value = (rand()%(high_stack_value+1-low_stack_value))+low_stack_value ;
  //   prev_stack_value[rnti]  = modified_stack_value; 
  //   cnt = 0; 
  //   //printf("phy rnti: %i, cqi %i \n",  rnti, modified_stack_value); 
  // } 

  switch (cqi_cfg.type) {
    case SRSRAN_CQI_TYPE_WIDEBAND:
      stack_value = cqi_value.wideband.wideband_cqi;
      // stack_value = modified_stack_value ; 

      /*
      //uint32_t min_cqi = cqi_min_max_vals[0];
      //uint32_t max_cqi = cqi_min_max_vals[1];
      //cqi_value = (rand()%(max_cqi+1-min_cqi))+min_cqi;
      
      printf("SRSRAN_CQI_TYPE_WIDEBAND\n"); 

            /// Harish code modifications: To configure CQI for each user based on the given range of cqis
      // Start
      
      //#if 1
      uint16_t flag = 1;  
      uint32_t ue_db_cnt =  2 ; //ue_db.size();
      if (flag){
        uint32_t min_cqi = cqi_min_max_vals[0];
        uint32_t max_cqi = cqi_min_max_vals[1];
        //cqi_value = (rand()%(max_cqi+1-min_cqi))+min_cqi;
        stack_value = (rand()%(max_cqi+1-min_cqi))+min_cqi ;

        if(cqi_disp_cnt<1)
        {
          printf("min_cqi %d max_cqi %d\n",min_cqi,max_cqi);
          cqi_disp_cnt++;
        }
      }
      //#else
      else{
        size_t cqi_arr_size = sizeof(cqi_min_max_vals)/sizeof(cqi_min_max_vals[0]);
        if(2*ue_cnt+1<=cqi_arr_size && ue_cnt<=ue_db_cnt)
        {
          uint32_t min_cqi = cqi_min_max_vals[ue_cnt*2];
          uint32_t max_cqi = cqi_min_max_vals[ue_cnt*2+1];
          stack_value = (rand()%(max_cqi+1-min_cqi))+min_cqi;
          if(cqi_disp_cnt<1)
          {
            printf("min_cqi %d max_cqi %d\n",min_cqi,max_cqi);
