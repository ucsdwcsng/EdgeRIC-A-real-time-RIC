               ...
               rrc_interface_mac*       rrc)
{
  ...
  char filename[440];
  if(cqi_change_flag)
    sprintf(filename,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/bb_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
  else
    sprintf(filename,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/bb_%s.txt",num_ues,algo,load,scenario);
  fp = fopen(filename,"w+");
  printf("Backlog Buffer saving file %s is opened %p\n",filename,fp);

  char filename1[440];
  if(cqi_change_flag)
    sprintf(filename1,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/tpt_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
  else
    sprintf(filename1,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/tpt_%s.txt",num_ues,algo,load,scenario);
  fp1 = fopen(filename1,"w+");
  printf("Throughput saving file %s is opened %p\n",filename1,fp1);
