...
//int cqi_range[2] = {5,10};
// int min_cqi = 5, max_cqi = 15;
//int cqi_range1[2] = {1,15};  //  if cqi_flag == 1  // use this for ue1
//int cqi_range2[2] = {1,10};  // if cqi_flag != 1  // use this for ue2
int cqi_range[2] = {1,15};  //initialized by params.txt
int8_t cqi_flag = 1;
int cqi_value = 15;   //  // initialized by params.txt  use this
// int cqi_offset_val = 0;
// int cqi_offset[2] = {-2,2};
bool cqi_evol_flag = 1;
int period_to_alter_range = 1000; // [msec]  // initialized by params.txt  // use this
int cqi_range_step = 0;
int cqi_change_cnt = 0;
// bool random_gen_flag = 0;
int cqi_disp_cnt1 = 0;
...
int srsran_ue_dl_init(srsran_ue_dl_t* q, cf_t* in_buffer[SRSRAN_MAX_PORTS], uint32_t max_prb, uint32_t nof_rx_antennas)
{
  ...
  if ((fptr = fopen("../params.txt","r")) == NULL){
       printf("Error! opening file");

       // Program exits if the file pointer returns NULL.
       //exit(1);
   }
   fscanf(fptr,"%d", &period);
   fscanf(fptr,"%d %d", &min_cqi, &max_cqi);
   fscanf(fptr,"%d", &init_cqi); 

   
   fclose(fptr); 

   period_to_alter_range = period ; 
   cqi_range[0] = min_cqi; 
   cqi_range[1] = max_cqi; 
   cqi_value = init_cqi; 
  printf("\n\n ue1: cqi period = %d msec, cqi range: (%d, %d), cqi init: %d \n\n", period, cqi_range[0], cqi_range[1], cqi_value);

  //out_mac_realistic_spin_3_cqi.txt
  FILE *fp_cqi; 
  
  int idx = 0;
  int cqi_c = 0 ;  
  
  fp_cqi = fopen("../cqis/out_mac_realistic_spin_3_cqi.txt", "r"); 
  ...
  //printf("__cqis[%d] %d", idx-1, __cqis[idx-1]); 
  
  
  ///// moved by whko 09062022
  ...
  char filename[440];
  // sprintf(filename,"/home/afc/Vamsi/5G_EdgeRIC_Updated/MobiCom_Results/%dUEs/%s/SL_%0.1fM_CQI_CHANGE_1IN%d_STEP%d/tpt_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_step_size,scenario);
  if(cqi_change_flag)
    sprintf(filename,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/cqi_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
  else
    sprintf(filename,"/home/wcsng-24/Ushasi3/EdgeRIC_main/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/cqi_ue1_%s.txt",num_ues,algo,load,scenario);

  // printf("filename %s",filename);
  if(firstwrite)
  {
    fp = fopen(filename,"w+");
    printf("CQI saving file opened %p\n",fp);
    firstwrite=0;
  }
  else
  {
                                   ...
                                   srsran_uci_data_t*  uci_data)
{
  ...
  // if(cqi_evol_flag)
  // {
  //   if(cqi_change_cnt==period_to_alter_range)
  //   {
  //     cqi_range[0] = (rand()%(max_cqi-min_cqi+1))+min_cqi;
  //     cqi_range[1] = cqi_range[0]+cqi_range_step;
  //     if(cqi_range[1]>max_cqi)
  //       cqi_range[1] = max_cqi;
  //     cqi_change_cnt = 0;
  //   }
  //   cqi_change_cnt++;
  //   if(cqi_value==0)
  //   {
  //     cqi_value = (rand()%(cqi_range[1]+1-cqi_range[0]))+cqi_range[0];
  //   }
  //   cqi_offset_val = (rand()%(cqi_offset[1]+1-cqi_offset[0]))+cqi_offset[0];
  //   cqi_value = cqi_value+cqi_offset_val;
  //   if(cqi_value>cqi_range[1]) cqi_value = cqi_range[1];
  //   else if (cqi_value<cqi_range[0]) cqi_value = cqi_range[0];

  //   if(cqi_value<1 || cqi_value>15){
  //     printf("CQI going out of range\n");
  //     exit(1);
  //   }
  //   wideband_value = cqi_value;
  //   // cqi_disp_cnt1++;
  //   // if(cqi_disp_cnt1>=1000)
  //   // {
  //   //   printf("cqi range [%d %d]\n",cqi_range[0],cqi_range[1]);
  //   //   cqi_disp_cnt1 = 0;
  //   // }
  // }
  ...
  if(cqi_evol_flag)
  {
    if (cqi_range[0] == 0 && cqi_range[1] == 0){
      cqi_value = __cqis[__idx_cqis++] ; 
      if (__idx_cqis >= __sz_cqis) __idx_cqis = 0; 
    }
    else{
      if(cqi_change_cnt==period_to_alter_range)
      {
        /*
        if(cqi_flag==1)
          cqi_value = (rand()%(cqi_range1[1]+1-cqi_range1[0]))+cqi_range1[0];
        else
          cqi_value = (rand()%(cqi_range2[1]+1-cqi_range2[0]))+cqi_range2[0];
        cqi_flag = (rand()%2)+1;
        */
        cqi_value-=cqi_range_step ; 
        cqi_change_cnt = 0;
      }
      cqi_change_cnt++;
      ...
      if (cqi_value <= cqi_range[0]) cqi_range_step = -1 ; 
      if (cqi_value >= cqi_range[1]) cqi_range_step = 1 ;
 ...
 /*
  ...
  // cqi_disp_cnt1++;
  // if(cqi_disp_cnt1>=500)
  // {
  //   printf("cqi_value %d\n",uci_data->value.cqi.wideband.wideband_cqi);
  //   cqi_disp_cnt1 = 0;
  // }
