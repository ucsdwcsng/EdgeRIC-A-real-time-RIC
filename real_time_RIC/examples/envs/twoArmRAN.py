...
class twoArmRAN(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, seed, numEpisodes,  Training, r1,  high_RB, low_RB, maxBufferSize, batchSize, episodeLimit, fixedSizeMDP, noiseVar, cost=0, zmq_params=None):

        super(twoArmRAN, self).__init__()
        # self.seed = seed
        self.cost = cost
    ...
    def _calReward(self, action, curState, nextState):
        ...
        total_reward = -(nextState[0] + nextState[2])*1e-6
    ...
    def send_action(self, action):
        ...
        weights = [RNTI1, weight1, RNTI2, weight2]
        #print(weights)
    ...
    def send_weight(self, weights):
        ...
        print("str_to_send_cur: ", str_to_send_cur)
    ...
    def reset(self):
        ...
        initialState = np.array(
            [self.arm[0][1], self.arm[0][2], self.arm[0][4],self.arm[0][5]], dtype=np.double)
    ...
    def get_state_ori(self):
        ...
        messagedata = string.split()  # [ rnti,cqi,rnti,cqi ]
        # print("mark1 crossed")
        # self.episodeTime = 0
        RNTI1 = self.arm[0][0]            
        CQI1 = self.arm[0][1]
        ...
        if len(messagedata) >= 6:
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Raini - extract CQI
            RNTI1 = int(messagedata[0])
            if int(messagedata[1]) > 0 : CQI1 =  int(messagedata[1]) 
            BL1 = int(messagedata[2])
            RNTI2 = int(messagedata[3])
            if int(messagedata[4]) > 0 : CQI2 =  int(messagedata[4])
            BL2 = int(messagedata[5])
                
        
        # print(CQI,"<-CQI"+20*"-")
        # print(MB1, MB2,"<-MB"+20*"-")

        return RNTI1, CQI1, BL1, RNTI2, CQI2, BL2

    #def _calReward(self, media_buffer1, media_buffer2):
    def _calReward_ori(self, backlog1, backlog2):
        ''' function to calculate next state, and reward given the action'''
        reward = 0
        if backlog1 <= 0.01*self.maxBufferSize:  # TODO ask Woo-Hyun to make UDP_client.py implement a fixed maxBufferSize and so that I can set this variable accordingly
            reward += 0
        else:
            reward += 1

        if backlog2 <= 0.01*self.maxBufferSize:  # TODO ask Woo-Hyun to make UDP_client.py implement a fixed maxBufferSize and so that I can set this variable accordingly
            reward += 0
        else:
            reward += 1
