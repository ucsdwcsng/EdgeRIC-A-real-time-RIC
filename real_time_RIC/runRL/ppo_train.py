...
class MaxCQIAgent:
    def __init__(self, augment_state_space) -> None:
        self.augmented_state_space = augment_state_space

    def restore(self, arg1):
        pass

    def select_action(self, obs):
        cqis = obs[1::3] if self.augmented_state_space else obs[1::3]
        alloc_mat = np.eye(len(cqis)) * 0.6 + 0.1
        return alloc_mat[np.argmax(cqis)]
...
class MaxPressureAgent:
    ...
    def select_action(self, obs):
        if self.augmented_state_space:
            pressure = obs[2::3]
        else:
            backlog_lens = obs[::3]
            #backlog_lens = backlog_lens/np.sum(backlog_lens)
            cqis = obs[1::3]
            print("cqis")
            print(cqis)
            #cqis = cqis/np.sum(cqis)
            if (min(cqis) > -1): 
                print("hey")
                sum_CQI = np.sum(cqis)
                sum_BL = np.sum(backlog_lens) 
                if (sum_BL != 0):
                    pressure = (cqis/sum_CQI) * (backlog_lens/sum_BL)
                    print(pressure)
                    
                else:
                    pressure = cqis/sum_CQI
            #pressure = [b * c for b, c in zip(backlog_lens, cqis)]
            #pressure = [(b * c)/ (sum(backlog_lens)*sum(cqis)) for b, c in zip(backlog_lens, cqis)]
            #print(pressure)
        #return np.eye(len(pressure))[np.argmax(pressure)]
        return pressure
