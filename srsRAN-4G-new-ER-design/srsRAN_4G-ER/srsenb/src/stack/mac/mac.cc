...
// Publisher for EdgeRIC
zmq::context_t context_main(1);
zmq::socket_t  publisher_main(context_main, ZMQ_PUB);

// Publisher for DL scheduling
zmq::socket_t publisher_dl_sched(context_main, ZMQ_PUB);

// Publisher for UL blanking
zmq::socket_t publisher_ul_blank(context_main, ZMQ_PUB);

// Subscriber from EdgeRIC
zmq::socket_t subscriber_edgeric(context_main, ZMQ_SUB);

// Subscriber for UL pending data
zmq::socket_t subscriber_ul_pd(context_main, ZMQ_SUB);

int                          tti_ran_index = 0;
uint32_t er_ran_index = 0;
std::map<uint16_t, float>    snr_ul_ues;
std::map<uint16_t, float>    cqi_dl_ues;
               ...
               rrc_interface_mac*       rrc)
{
  ...
  publisher_dl_sched.bind("ipc:///tmp/socket_dlscheduling");
  publisher_ul_blank.bind("ipc:///tmp/socket_blanking");
  
  // Connect ZMQ subscriber and subscribe to messages from edgeric
  subscriber_edgeric.connect("ipc:///tmp/control_actions"); // Change this to your ZMQ server address
  subscriber_edgeric.setsockopt(ZMQ_SUBSCRIBE, "", 0);

  // Set the socket to conflate mode
  int conflate2 = 1;
  subscriber_edgeric.setsockopt(ZMQ_CONFLATE, &conflate2, sizeof(conflate2));

  // Connect ZMQ subscriber and subscribe to messages for UL Pending Data
  subscriber_ul_pd.connect("ipc:///tmp/socket_ul_pending_data"); // Change this to your ZMQ server address
  subscriber_ul_pd.setsockopt(ZMQ_SUBSCRIBE, "", 0);

  // Set the socket to conflate mode
  int conflate = 1;
  subscriber_edgeric.setsockopt(ZMQ_CONFLATE, &conflate, sizeof(conflate));
...
void mac::ric_comm()
{
  ...
  tti_ran_index += 1;
  std::string send_message_to_dl_scheduler;
  std::string send_message_to_ul_blanker;
  // tti_ran_index = tti_ran_index%2000;
  ////////////////// Receive from EdgeRIC
  // Open a file to log the received messages and parsed data
    std::ofstream loggerfile("log_output.txt", std::ios::out | std::ios::app);  // Open in append mode
  ...
  zmq_setsockopt(subscriber_edgeric, ZMQ_SUBSCRIBE, "", 0);
  auto size = subscriber_edgeric.recv(recv_message_er, zmq::recv_flags::dontwait);

  // Log the received message
  //loggerfile << "Received on zmq: " << recv_message_er << std::endl;
  //loggerfile << " Current Ran index is " << tti_ran_index << std::endl;
  ...
  if (size) {
    ...
    // Populate the weights_recved map/ blanking a and b
    for (size_t i = 1; i < tokens.size(); i += 2) {
      if (i == 1) {er_ran_index = std::stoi(tokens[i - 1]); 
      //loggerfile << "time: " << std::fixed << std::setprecision(5) << time_in_seconds << "s" << ", Received index from ER: " << er_ran_index << std::endl;
      }
      if (i + 1 < tokens.size() - 2) {
        uint16_t rnti        = std::stoi(tokens[i]);
        float    weight      = std::stof(tokens[i + 1]);
        weights_recved[rnti] = weight;
        send_message_to_dl_scheduler += std::to_string(rnti) + " " + std::to_string(weight) + " ";
        //loggerfile << " Weight of RNTI " << rnti << " is " << weight;
      } else {
        uint16_t a = std::stoi(tokens[i]);
        uint16_t b = std::stoi(tokens[i + 1]);
        send_message_to_ul_blanker += std::to_string(a) + " " + std::to_string(b);
        //loggerfile << " a and b are: " << a << " " << b << "\n";
      }
    }
    send_message_to_dl_scheduler += std::to_string(er_ran_index) + " ";

     ////////////////////// Publish the control actions
    int            m_size = send_message_to_dl_scheduler.size();
    zmq::message_t message(m_size);
    snprintf((char*)message.data(), m_size, "%s", send_message_to_dl_scheduler.c_str());
    publisher_dl_sched.send(message, zmq::send_flags::none);

    int            m_size2 = send_message_to_ul_blanker.size();
    zmq::message_t message2(m_size2);
    snprintf((char*)message2.data(), m_size2, "%s", send_message_to_ul_blanker.c_str());
    publisher_ul_blank.send(message2, zmq::send_flags::none);
  ...
  zmq_setsockopt(subscriber_ul_pd, ZMQ_SUBSCRIBE, "", 0);
  auto size2 = subscriber_ul_pd.recv(recv_message, zmq::recv_flags::dontwait);
  ...
  if (size2) {
    ...
    for (size_t i = 0; i < tokens.size(); i += 2) {
      if (i + 1 < tokens.size()) {
        uint16_t rnti      = std::stoi(tokens[i]);
        uint32_t pd        = std::stoi(tokens[i + 1]);
        pending_data[rnti] = pd;
      }
    }
  }
  ///////////////////////////////////
  ...
  std::ofstream logFile("ue_allocations.log", std::ios::out | std::ios::app);
  if (!logFile.is_open()) {
    std::cerr << "Failed to open log file." << std::endl;
    return;
  }
  ...
  for (auto& ue_pair : ue_db) {
    ...
    cqi_dl_ues[rnti] = cqi ? cqi : cqi_dl_ues[rnti]; // if cqi is not zero, do update
    cqi              = cqi_dl_ues[rnti];
    // uint8_t cqi = cqi_dl_ues[rnti]; //<uint8_t>(ue_metrics.dl_cqi);
    float    snr_ue = snr_ul_ues[rnti];
    float    rxb    = rx_bytes_ues[rnti];
    float    txb    = tx_bytes_ues[rnti];
    uint32_t pd     = pending_data[rnti];
    ...
    //logFile << "time: " << std::fixed << std::setprecision(5) << time_in_seconds << "s" << " "
            // << "RNTI: " << static_cast<int>(rnti) << ", TTI: " << tti_ran_index << ", Received ER index: " << er_ran_index << ", snr_ue: " << snr_ue
            // << ", CQI: " << cqi << ", Backlog: " << backlog << ", rx_brate: " << rxb << ", pending data: " << pd
            // << ", tx_brate: " << txb << std::endl;
    // << ", rx_errors: " << static_cast<int>(rx_errors)
    // << ", rx_packets: " << static_cast<int>(rx_packets)
    // << ", UL buffer: " << static_cast<int>(ue_buf) << std::endl;

    send_message_to_er += std::to_string(rnti) + " " + std::to_string(cqi) + " " + std::to_string(backlog) + " " +
                          std::to_string(txb) + " " + std::to_string(rxb) + " " + std::to_string(pd) + " " +
                          std::to_string(snr_ue) + " ";
  }

  logFile.close();

  // Finalize and send the message using ZMQ publisher
  int            m_size3 = send_message_to_er.size();
  zmq::message_t message3(m_size3);
  snprintf((char*)message3.data(), m_size3, "%s", send_message_to_er.c_str());
  publisher_main.send(message3, zmq::send_flags::none);
}
