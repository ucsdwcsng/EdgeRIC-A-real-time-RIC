...
rbgmask_t           prev_mask;
uint16_t prev_rnti  = 0 ; 
//std::map<uint16_t, uint16_t> weights ; 
//alloc_result try_dl_newtx_alloc_greedy(sf_sched& tti_sched, sched_ue& ue, const dl_harq_proc& h, rbgmask_t* result_mask)
alloc_result try_dl_newtx_alloc_greedy(sf_sched& tti_sched, sched_ue& ue, const dl_harq_proc& h, rbgmask_t* result_mask, uint16_t rbg_alloc)
{
  ...
  // rbg allocation by weights: whko 08112022
  if (rbg_alloc > 0){
    std::string s;
    //for (size_t i = opt_mask.size(); i > 0; --i) {
    for (size_t i = 0; i < opt_mask.size(); i++) {
      s.append(opt_mask.test(i) ? "1" : "0");

      
      if(opt_mask.test(i) && rbg_alloc > 0){
        rbg_alloc--; 
      }
      else{
        opt_mask.set(i, false); 
      }

    }
    //prev_rnti = ue.get_rnti() ; 
    //printf("ue: %x  opt_mask: %s \n", ue.get_rnti(), s.c_str());
    ///
  }
  //else  printf(" rbg_alloc of rnti %i is 0.\n",ue.get_rnti()); 
  
  // empty RBGs were found. Attempt allocation
  alloc_result ret = tti_sched.alloc_dl_user(&ue, opt_mask, h.get_id());
  // 6. RAN does RB allocation
  std::ofstream outfile_seq ;
  outfile_seq.open("out_mac_seq_1.txt", ios::out | ios::app); 
  struct timespec ctime ; 
  int ret_6 =clock_gettime(CLOCK_REALTIME, &ctime); 
  long utime_cur_system = ctime.tv_sec*1000000000 + ctime.tv_nsec ; 
  outfile_seq << std::to_string(utime_cur_system) + ",\t" +  "6\n"; 
