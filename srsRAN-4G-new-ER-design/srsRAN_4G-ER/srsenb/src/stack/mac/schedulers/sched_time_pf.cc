...
{ uint8_t num_ues = ue_db.size();
  ...
  zmq::message_t recv_message2;
  zmq_setsockopt(subscriber2, ZMQ_SUBSCRIBE, "", 0);
  auto size_dl = subscriber2.recv(recv_message2, zmq::recv_flags::dontwait);
    
  if (size_dl) {
      std::string received_str(static_cast<char*>(recv_message2.data()), recv_message2.size());
      // printf("I received weight \n");
      // printf("Received string: %s\n", received_str.c_str());

      // Initialize the map to store the weights
      std::map<uint16_t, float> weights_recved;
      ...
      // Populate the weights_recved map
      for (size_t i = 0; i < tokens.size(); i += 2) {
          if (i + 1 < tokens.size()) {
            ...
            uint32_t ran_index = std::stoi(tokens[i]);
            std::ofstream log_file("ue_allocations.log", std::ios::app);
              if (log_file.is_open()) {
              log_file << "time: " << std::fixed << std::setprecision(5) << time_in_seconds << "s" << " "
                       << "Acting on TTI " << ran_index << '\n';
                  
              log_file.close(); // Close the file after writing to it
            }
          }
      }
      // Print the parsed weights
        int j = 0;
