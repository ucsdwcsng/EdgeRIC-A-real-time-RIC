...
bool cqi_evol_flag = 0;  // 1: using cqi emulation, 0: using reported cqi  set by args --params_filename 
int period_to_alter_range = 40; // [msec]  // initialized by params.txt  // use this
...
int srsran_ue_dl_init_emul(srsran_ue_dl_t* q, cf_t* in_buffer[SRSRAN_MAX_PORTS], uint32_t max_prb, uint32_t nof_rx_antennas, const char* params_filename)
{
  ...
  //if ((fptr = fopen("../params.txt","r")) == NULL){
   if ((fptr = fopen(params_filename,"r")) == NULL){
  ...
  printf("\n\n ue: cqi period = %d msec, cqi range: (%d, %d), cqi init: %d \n\n cqi file: %s \n\n # of cqis: %d ", period, cqi_range[0], cqi_range[1], cqi_value, cqi_filename, __sz_cqis);
                                   ...
                                   srsran_uci_data_t*  uci_data)
{
  ...
  if(cqi_evol_flag)
  {
    ...
    // override wideband_value with emulation value
