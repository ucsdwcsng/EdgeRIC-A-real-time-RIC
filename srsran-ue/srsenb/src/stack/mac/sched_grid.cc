...
// added by whko 10252022
zmq::context_t context_1(1);
zmq::socket_t publisher_log(context_1, ZMQ_PUB) ;


void sf_sched::logging(std::string msg)
{

  
  int m_size = msg.size(); 
  zmq::message_t message(m_size); 
  snprintf((char *)message.data(), m_size, msg.c_str(), NULL ); 
  publisher_log.send(message, zmq::send_flags::none ); 
  //printf("logging seq 6\n"); 
...
void sf_sched::init(const sched_cell_params_t& cell_params_)
{
  ...
  // added by whko 10252022
  //printf("******************sched_grid init **************\n");
  publisher_log.bind("ipc:///tmp/socket_logging_seq_6"); 
