                                        ...
                                        bool                 print_carrier_num)
{
  ...
  if(log_flag)
  {
    ...
    if(cqi_change_flag)
      sprintf(filename,"/home/afc/git/5G_EdgeRIC_Updated/MobiCom_Results/%dUEs/%s/CQI_CHANGE/SL_%0.1fM_CQI_CHANGE_1IN%d_RSTEP%d/tpt_ue1_%s.txt",num_ues,algo,load,cqi_change_period,cqi_range_size,scenario);
    else
      sprintf(filename,"/home/afc/git/5G_EdgeRIC_Updated/MobiCom_Results/%dUEs/%s/NO_CQI_CHANGE/SL_%0.1fM_NO_CQI_CHANGE/tpt_ue1_%s.txt",num_ues,algo,load,scenario);
