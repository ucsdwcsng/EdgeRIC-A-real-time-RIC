  ...
  logger(logger)
{
  ...
  if (phy->args->params_filename == "auto"){
    ...
    if (srsran_ue_dl_init_emul(&ue_dl, signal_buffer_rx, max_prb, phy->args->nof_rx_ant, phy->args->params_filename.c_str())) { // whko 08312023
      Error("Initiating UE DL");
      return;
    }
  }
