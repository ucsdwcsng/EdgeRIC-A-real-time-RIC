...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  // clang-format off
  common.add_options()
    ("params_filename", bpo::value<string>(&args->phy.params_filename)->default_value("auto"), "params filename")
  ...
  if (args->stack.rrc_nr.scs == srsran_subcarrier_spacing_invalid ||
   ...
   // read a cqi file
  if (args->phy.params_filename != "auto") {
    printf("params filename : %s \n", args->phy.params_filename.c_str());
  } 
